...
# SCPN Muon Fusion Core — reactor-domain validator tests
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-MUON-FUSION-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-MUON-FUSION-CORE"
                },
...
def test_namespaced_identifiers_are_admitted_and_bare_colons_are_not(
    tmp_path: Path,
) -> None:
    """The extension form ``namespace.part:name`` passes; malformed forms fail."""
    accepted = write_manifest(
        tmp_path, mutated(configurations=["group.part:name_one", "plain_name"])
    )
    assert not any(
        "invalid identifier" in finding for finding in validate_manifest(accepted, None)
    )
    for bad in (":name", "Group:name", "group:Name", "group..part:name", "group:"):
        rejected = write_manifest(tmp_path, mutated(configurations=[bad]))
        assert any(
            "invalid identifier" in finding
            for finding in validate_manifest(rejected, None)
        ), bad


def test_map_cross_check_accepts_the_pending_registry_extension(
    tmp_path: Path,
) -> None:
    """A manifest pinning the prepared release validates against the pending block."""
    manifest = load_json_object(MANIFEST)
    map_path = tmp_path / "map.json"
    map_path.write_text(
        json.dumps(
    ...
    drifted.write_text(
        map_path.read_text(encoding="utf-8").replace(
            manifest["spo_registry"]["digest_sha256"], "f" * 64
        ),
        encoding="utf-8",
    )
    findings = validate_manifest(MANIFEST, drifted)
    assert any("digest does not match" in finding for finding in findings)
    assert not any("registry version" in finding for finding in findings)
