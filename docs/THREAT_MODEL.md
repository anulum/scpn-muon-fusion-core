...
SCPN Muon Fusion Core — Threat model
-->
...
implementation, solver seam, federation) is added. One asset is specific
to this family and is listed first.

## Assets

| Asset | Why it matters |
|---|---|
| The family's non-claim of no energy gain | the muon-catalysed field is bounded by the muon's production cost against its catalysed yield; every surface must keep that arithmetic visible, and no descriptor may be read as promising a path around it |
| `reactor-domain.json` | source of project identity; downstream projects bind to it, including the pin of the prepared registry release `1.1.0` |
...
| `docs/CONTROL_ADAPTER_SPECIFICATION.md` | safety-relevant contract text (no-direct-actuation semantics; tritium, cryogenic and radiation envelope) |
...
|---|---|
| Presenting the family as an energy-gain path | the manifest's non-claims carry the explicit statement with the published numbers; README, ADR 0001 and every future model docstring repeat it; the validator enforces empty claims at `architecture_only` and evidence pointers afterwards |
...
| Re-pinning the SPO registry silently, or pinning a release the map does not carry | validator fails on version/digest mismatch against the family map's source registry or, for a pending release, against the map's pending block; the pending block's digest is the one computed from the prepared patch |
...
- Until the orchestrator lands registry `1.1.0`, the pin is to a prepared
  release; if the landed digest differs, the manifest is re-pinned in one
  governed commit and the interval is visible in the family map.
